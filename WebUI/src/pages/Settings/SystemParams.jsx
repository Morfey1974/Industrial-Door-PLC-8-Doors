    ...
    <div className="settings-system-params">
      ...
      <section className="system-params-section system-params-section-vpn">
        ...
        <div className="system-params-vpn-options">
          ...
          <ul>
            <li><strong>VPN на роутере:</strong> если роутер поддерживает WireGuard или OpenVPN — включите VPN-сервер в веб-интерфейсе роутера, создайте пользователя/ключ. С телефона или ноутбука из другого города подключайтесь к этому VPN — затем откройте <code>http://IP_ПК:3000</code> и в «Подключение к контроллеру» укажите <code>http://IP_контроллера:порт</code> или <code>http://IP_ПК:3080</code> (если используете прокси).</li>
            <li><strong>VPN на ПК:</strong> на компьютере, где запущен WebUI (и прокси при необходимости), установите VPN-сервер (WireGuard, OpenVPN). На роутере настройте переадресацию порта VPN на этот ПК. С удалённого устройства подключайтесь к VPN по внешнему IP или динамическому DNS.</li>
            <li><strong>Tailscale / ZeroTier:</strong> установите на ПК в сети и на устройство, с которого подключаетесь. Сервис создаёт зашифрованный туннель без настройки роутера. После подключения используйте тот же IP ПК (например 100.x.x.x в Tailscale), порты 3000 и 3080.</li>
          </ul>
        </div>
        <div className="system-params-vpn-after">
          ...
          <ol>
            ...
            <li>Нажмите «Подключение к контроллеру» и укажите адрес контроллера (<code>http://192.168.1.50:80</code>) или прокси (<code>http://IP_ПК:3080</code>).</li>
