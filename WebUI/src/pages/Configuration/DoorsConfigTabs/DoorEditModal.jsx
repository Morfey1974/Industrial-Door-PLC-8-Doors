...
const DoorEditModal = ({ door, existingDoors, onSave, onCancel }) => {
  ...
  const validate = () => {
    ...
    if (!formData.techId || formData.techId < 1) {
      newErrors.techId = 'Порядковый номер должен быть положительным числом';
    } else {
      ...
      if (duplicate) {
        newErrors.techId = `Порядковый номер ${formData.techId} уже используется`;
      }
    }
    ...
    if (formData.comment && formData.comment.length > 100) {
      newErrors.comment = 'Комментарий не может быть длиннее 100 символов';
    }
        ...
        <div className="modal-body door-edit-modal-body">
          <div className="door-edit-grid door-edit-grid--row1">
            <div className="form-group">
              <label htmlFor="techId">Порядковый номер двери <span className="required">*</span></label>
              <input
                id="techId"
                type="number"
          ...
          <div className="door-edit-grid door-edit-grid--row2">
            <div className="form-group">
              <label htmlFor="nodeId">Выбор платы <span className="required">*</span></label>
              <select
                id="nodeId"
            ...
            <div className="form-group">
              <label htmlFor="localDoor">Выбор двери <span className="required">*</span></label>
              <select
                id="localDoor"
          ...
          <div className="door-edit-grid door-edit-grid--row3">
            <div className="form-group">
              <label htmlFor="type">Состояние двери <span className="required">*</span></label>
              <select
                id="type"
                value={formData.type}
